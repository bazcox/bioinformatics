...
new_row <- data.frame(matrix(NA, nrow = 1, ncol = 143))
...
for (col in setdiff(1:143, exclude_cols)) {
...
cols_with_2_classes <- which(sapply(seq_len(130), function(col_idx) length(unique(test.set[[col_idx]])) == 2))
...
elastic.pred <- predict(elastic, newdata = test.set[,1:142])
...
xgb.save(best.boost, "xgboost.gwas75.beforeBH.indiv.model.non-genetic.probe1")
...
#predict on test.set without last row of dummy variables
elastic.pred <- predict(elastic, newdata = test.set[,1:203])
...
set.seed(123)
best.boost <- xgboost(data=dtrain_mQTL, params=best.param, nrounds=nround, verbose = FALSE)
xgb.save(best.boost, "xgboost.gwas75.beforeBH.indiv.model.non-genetic.probe3")
...
## step 3
# Find columns in test.set that have only 2 unique values
cols_with_2_classes <- which(sapply(seq_len(150), function(col_idx) length(unique(test.set[[col_idx]])) == 2))
...
#predict on test.set without last row of dummy variables
elastic.pred <- predict(elastic, newdata = test.set[,1:162])
...
set.seed(123)
best.boost <- xgboost(data=dtrain_mQTL, params=best.param, nrounds=nround, verbose = FALSE)
xgb.save(best.boost, "xgboost.gwas75.beforeBH.indiv.model.non-genetic.probe4")
...
#predict on test.set without last row of dummy variables
elastic.pred <- predict(elastic, newdata = test.set[,1:183])
...
set.seed(123)
best.boost <- xgboost(data=dtrain_mQTL, params=best.param, nrounds=nround, verbose = FALSE)
xgb.save(best.boost, "xgboost.gwas75.beforeBH.indiv.model.non-genetic.probe5")
...
# Create a new row with all missing values
new_row <- data.frame(matrix(NA, nrow = 1, ncol = 167))
...
# Loop over each column, except the specific one, and randomly assign a value based on the existing classes
for (col in setdiff(1:167, exclude_cols)) {
  existing_classes <- unique(train.set[, col])
  new_row[, col] <- existing_classes[ceiling(runif(1, min = 1, max = length(existing_classes)))]
}
...
## step 3
# Find columns in test.set that have only 2 unique values
cols_with_2_classes <- which(sapply(seq_len(154), function(col_idx) length(unique(test.set[[col_idx]])) == 2))
...
#predict on test.set without last row of dummy variables
elastic.pred <- predict(elastic, newdata = test.set[,1:166])
...
set.seed(123)
best.boost <- xgboost(data=dtrain_mQTL, params=best.param, nrounds=nround, verbose = FALSE)
xgb.save(best.boost, "xgboost.gwas75.beforeBH.indiv.model.non-genetic.probe7")
