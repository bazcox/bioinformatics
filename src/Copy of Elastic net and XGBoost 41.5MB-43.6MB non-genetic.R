...
GenotypeFile <- read.csv("08-41.5MB-43.6MBsnps_MAF10_genotype.csv")
...
new_row <- data.frame(matrix(NA, nrow = 1, ncol = 2420))
...
#                     actual column + 1
new_row[, 732] <- "A A" #731 + 1
new_row[, 733] <- "G G" #732 + 1
new_row[, 1178] <- "T T" #1177 + 1
new_row[, 1242] <- "C C" #1241 + 1
new_row[, 1952] <- "A A" #1951 + 1
new_row[, 2060] <- "A A" #2059 + 1
...
exclude_cols <- c(732, 733, 1178,1242,1952,2060,2148,2170,2183,2188,2406)
# Loop over each column, except the specific one, and randomly assign a value based on the existing classes
for (col in setdiff(1:2420, exclude_cols)) {
...
cols_with_2_classes <- which(sapply(seq_len(2407), function(col_idx) length(unique(test.set[[col_idx]])) == 2))
...
elastic.pred <- predict(elastic, newdata = test.set[,1:2419])
...
xgb.save(best.boost, "xgboost.41.5MB-43.6MB.model.non-genetic.probe1")
...
exclude_cols <- c(732, 733, 1178,1952,2060,2148,2170,2183,2188,2406)
# Loop over each column, except the specific one, and randomly assign a value based on the existing classes
for (col in setdiff(1:2420, exclude_cols)) {
  existing_classes <- unique(train.set[, col])
  new_row[, col] <- existing_classes[ceiling(runif(1, min = 1, max = length(existing_classes)))]
...
set.seed(123)
best.boost <- xgboost(data=dtrain_mQTL, params=best.param, nrounds=nround, verbose = FALSE)
xgb.save(best.boost, "xgboost.41.5MB-43.6MB.model.non-genetic.probe3")
...
set.seed(123)
best.boost <- xgboost(data=dtrain_mQTL, params=best.param, nrounds=nround, verbose = FALSE)
xgb.save(best.boost, "xgboost.41.5MB-43.6MB.model.non-genetic.probe4")
...
exclude_cols <- c(732, 733, 1178,1952,2148,2170,2183,2188,2406)
# Loop over each column, except the specific one, and randomly assign a value based on the existing classes
for (col in setdiff(1:2420, exclude_cols)) {
  existing_classes <- unique(train.set[, col])
  new_row[, col] <- existing_classes[ceiling(runif(1, min = 1, max = length(existing_classes)))]
...
set.seed(123)
best.boost <- xgboost(data=dtrain_mQTL, params=best.param, nrounds=nround, verbose = FALSE)
xgb.save(best.boost, "xgboost.41.5MB-43.6MB.model.non-genetic.probe5")
...
set.seed(123)
best.boost <- xgboost(data=dtrain_mQTL, params=best.param, nrounds=nround, verbose = FALSE)
xgb.save(best.boost, "xgboost.41.5MB-43.6MB.model.non-genetic.probe7")
