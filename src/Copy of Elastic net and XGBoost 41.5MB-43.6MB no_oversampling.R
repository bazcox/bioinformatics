...
exclude_cols <- c(1178,1952,2060,2148,2170,2183,2188,2406)
...
elastic.pred <- predict(elastic, newdata = test.set[,1:2406])
...
xgb.save(best.boost, "xgboost.41.5MB-43.6MB.model.probe1")
...
exclude_cols <- c(1178,1952,2148,2170,2183,2188,2406)
# Loop over each column, except the specific one, and randomly assign a value based on the existing classes
for (col in setdiff(1:2407, exclude_cols)) {
  existing_classes <- unique(train.set[, col])
  new_row[, col] <- existing_classes[ceiling(runif(1, min = 1, max = length(existing_classes)))]
...
set.seed(123)
best.boost <- xgboost(data=dtrain_mQTL, params=best.param, nrounds=nround, verbose = FALSE)
xgb.save(best.boost, "xgboost.41.5MB-43.6MB.model.probe3")
...
set.seed(123)
best.boost <- xgboost(data=dtrain_mQTL, params=best.param, nrounds=nround, verbose = FALSE)
xgb.save(best.boost, "xgboost.41.5MB-43.6MB.model.probe4")
...
set.seed(123)
best.boost <- xgboost(data=dtrain_mQTL, params=best.param, nrounds=nround, verbose = FALSE)
xgb.save(best.boost, "xgboost.41.5MB-43.6MB.model.probe5")
...
exclude_cols <- c(1178,1242,1952,2060,2148,2170,2183,2188,2406)
# Loop over each column, except the specific one, and randomly assign a value based on the existing classes
for (col in setdiff(1:2407, exclude_cols)) {
  existing_classes <- unique(train.set[, col])
  new_row[, col] <- existing_classes[ceiling(runif(1, min = 1, max = length(existing_classes)))]
...
set.seed(123)
best.boost <- xgboost(data=dtrain_mQTL, params=best.param, nrounds=nround, verbose = FALSE)
xgb.save(best.boost, "xgboost.41.5MB-43.6MB.model.probe7")
