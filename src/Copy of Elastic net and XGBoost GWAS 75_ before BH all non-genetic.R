...
new_row <- data.frame(matrix(NA, nrow = 1, ncol = 997))
...
for (col in setdiff(1:997, exclude_cols)) {
...
cols_with_2_classes <- which(sapply(seq_len(984), function(col_idx) length(unique(test.set[[col_idx]])) == 2))
...
elastic.pred <- predict(elastic, newdata = test.set[,1:996])
...
xgb.save(best.boost, "xgboost.gwas75.beforeBH.all.model.non-genetic.probe1")
...
set.seed(123)
best.boost <- xgboost(data=dtrain_mQTL, params=best.param, nrounds=nround, verbose = FALSE)
xgb.save(best.boost, "xgboost.gwas75.beforeBH.all.model.non-genetic.probe3")
...
set.seed(123)
best.boost <- xgboost(data=dtrain_mQTL, params=best.param, nrounds=nround, verbose = FALSE)
xgb.save(best.boost, "xgboost.gwas75.beforeBH.all.model.non-genetic.probe4")
...
set.seed(123)
best.boost <- xgboost(data=dtrain_mQTL, params=best.param, nrounds=nround, verbose = FALSE)
xgb.save(best.boost, "xgboost.gwas75.beforeBH.all.model.non-genetic.probe5")
...
set.seed(123)
best.boost <- xgboost(data=dtrain_mQTL, params=best.param, nrounds=nround, verbose = FALSE)
xgb.save(best.boost, "xgboost.gwas75.beforeBH.all.model.non-genetic.probe7")
